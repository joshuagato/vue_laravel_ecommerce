    ...
    <section id="viewed-products" ref="viewed-products">
        ...
        <template>
            <section
                ...
                v-if="viewedproducts && viewedproducts.length > 0"
            >
                <carousel v-bind="settings">
                    <div
                        ref="viewed"
                        tabindex="0"
                        class="viewed activeviewed"
                        v-for="prod in viewedproducts"
                        :key="prod.id"
                    >
                        <div class="img-container">
                            <img
                                :src="
                                    `${imageUrl}${getAttribute(
                                        prod.product,
                                        'image'
                                    )}`
                                "
                        ...
                        <div class="details-container">
                            <p class="product-category text-center mb-1">
                                <span class="pl-1">{{
                                    fetchOneCategoryMethod(
                                        getAttribute(prod.product, "category")
                                    )
                                }}</span>
                            </p>
                            <h6 class="product-name text-center">
                                {{ getAttribute(prod.product, "title") }}
                            </h6>
                            <p class="product-price text-center">
                                ${{ getAttribute(prod.product, "price") }}
                            </p>
...
export default {
    ...
    data() {
        return {
            ...
            imageUrl: process.env.MIX_IMAGE_URL,
            settings: {
                autoplay: true,
    ...
    methods: {
        viewedProductsAnimation() {
            ...
            if (
                pageYOffset >
                    viewedSectionPositionFromTop -
                        (viewedSectionHeight * 99) / 100 ||
                scrollTopValue >
                    viewedSectionPositionFromTop -
                        (viewedSectionHeight * 99) / 100
            ) {
